...
biomass_Server <- function(id, all_dbi, region_names, input_region, input_species) {
  moduleServer(
    id,
      ...
      output$dbiPlot <- renderPlot({
        if (input_region() == "All regions") { # messages for no region or species 
          ...
          valid_dbi_surveys <- all_dbi |> 
            filter(common_name == input_species(), survey %in% input$surveys_selected)
          ...
          pdbi1 <- plot_dbi(input_species(), input$surveys_selected)
          pdbi2 <- plot_stan_dbi(input_species(), input$surveys_selected)
      ...
      observe(
        if (input_region() == "All regions") {
          ...
          output$downloadBiomass <- downloadHandler(
            ...
            content = function(file) {ggsave(file, plot =  plot_dbi(input_species(), input$surveys_selected), width = 10, device = "png")})
          output$downloadStanBiomass <- downloadHandler(
            filename = function() {paste0("stan_biomass_plot_", input_species(),"_", region_names(), ".png")},
            content = function(file) {ggsave(file, plot =  plot_stan_dbi(input_species(), input$surveys_selected), width = 10, device = "png")})
